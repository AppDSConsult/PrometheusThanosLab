...
resource "azurerm_virtual_network" "ubuntulab-vnet" {
  name                = "ubuntulab-vnet"
...
#Create new subnet
resource "azurerm_subnet" "ubuntulab-subnet" {
  name                 = "ubuntulab-subnet"
  resource_group_name  = var.resource_group_name
  virtual_network_name = azurerm_virtual_network.ubuntulab-vnet.name
...
resource "azurerm_public_ip" "ubuntulab-pip" {
  count               = var.vm_count
...
resource "azurerm_network_interface" "ubuntulab-nwi" {
  count               = var.vm_count
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.ubuntulab-pip.*.id[count.index] #Remove if public IP not needed
  }
...
network_interface_id      = azurerm_network_interface.ubuntulab-nwi.*.id[count.index]
...
resource "azurerm_virtual_machine" "main" {
  ...
  location              = var.region
  resource_group_name   = var.resource_group_name
  network_interface_ids = [azurerm_network_interface.ubuntulab-nwi.*.id[count.index]]
  vm_size               = var.vm_size
